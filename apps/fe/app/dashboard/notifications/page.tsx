...
export default function NotificationsPage() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/notifications${tab === "unread" ? "?unreadOnly=true" : ""}`, {
        cache: "no-store",
      })
      if (!res.ok) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      <AceternitySidebar items={dashboardItems} title="Laladulara" subtitle="User Dashboard" />
              ...
              {filtered.map((n) => (
                <div
                  ...
                  className={`rounded-xl border p-4 bg-white/5 ${
                    n.read ? "border-white/10" : "border-cyan-400/30"
                  }`}
                >
